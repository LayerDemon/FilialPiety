...
//            label.backgroundColor = [UIColor colorWithRed:255.0/tabbarItems.count*i/255 green:255.0/tabbarItems.count*i/255 blue:255.0/tabbarItems.count*i/255 alpha:1];
//            label.frame = CGRectMake((item.width-label.bounds.size.width)/2.0f, (height-label.bounds.size.height)/2.0f, CGRectGetWidth(label.bounds), CGRectGetHeight(label.bounds));
            label.frame = CGRectMake(0,0, item.width,height);
            
            [backView addSubview:label];
            UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapAction:)];
            [backView addGestureRecognizer:tap];

            [scrollView_ insertSubview:backView atIndex:0];
            x += item.width;
            i++;
        }
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    trackView_.frame = CGRectMake(x,self.bounds.size.height-2, width, CGRectGetHeight(trackView_.bounds));
}
...
//            }
            ...
            trackView_.frame = CGRectMake(trackRc.origin.x,self.bounds.size.height-2, trackRc.size.width, CGRectGetHeight(trackView_.bounds));
        }
